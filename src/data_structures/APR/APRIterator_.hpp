//
// Created by cheesema on 17.07.18.
//

#ifndef LIBAPR_APRITERATOR_HPP
#define LIBAPR_APRITERATOR_HPP
...
class APRIterator_  : public GenIterator {

public:

    explicit APRIterator_(APRAccess& apr_access_) {
    ...
    inline bool set_neighbour_iterator(APRIterator_ &original_iterator, const uint8_t& direction, const uint8_t& index);
protected:
...
uint64_t APRIterator_::start_index(const uint16_t level, const uint64_t offset){
...
uint64_t APRIterator_::max_row_level_offset(const uint16_t x,const uint16_t z,const uint16_t num_parts){
    return ((x%2) + (z%2)*2)*((uint64_t)num_parts) ;//calculates the number of particles in the row
}

inline uint64_t APRIterator_::set_new_lzx(const uint16_t level,const uint16_t z,const uint16_t x){
...
uint64_t APRIterator_::set_new_lzxy(const uint16_t level,const uint16_t z,const uint16_t x,const uint16_t y){
...
inline bool APRIterator_::set_iterator_to_particle_next_particle(){
    //
    //  Moves the iterator to point to the particle number (global index of the particle)
    //

    if( (this->current_particle_cell.y+1) <= this->apr_access->yg_end[this->current_gap.gap_index]){
        //  Still in same y gap
        ...
        //not in the same gap
        this->current_gap.gap_index++;//move the iterator forward.
...
inline bool APRIterator_::find_neighbours_in_direction(const uint8_t& direction){
...
inline bool APRIterator_::set_neighbour_iterator(APRIterator_ &original_iterator, const uint8_t& direction, const uint8_t& index){
    //
...
inline bool APRIterator_::set_iterator_by_particle_cell(ParticleCell& random_particle_cell){
    //
...
inline bool APRIterator_::set_iterator_by_global_coordinate(float x,float y,float z){
    //
...
inline bool APRIterator_::find_neighbours_same_level(const uint8_t& direction){
...
inline bool APRIterator_::find_next_child(const uint8_t& direction,const uint8_t& index){
